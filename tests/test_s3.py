...
class TestS3:
    ...
    def s3_test_setup(self, s3_conn: S3):
        ...
        s3_conn.write_json(
            target=JSON_FILE, content=JSON_FILE_CONTENT, bucket=TEST_BUCKET
        )
        s3_conn.write_to_file(filename=SUB_FOLDER_FILE, content=SUB_FOLDER_FILE_CONTENT)
        yield
    ...
    def test_repr(self, s3_conn: S3):
        assert s3_conn.__repr__().startswith(
            "<s3.s3.S3 object at"
        ) and s3_conn.__repr__().endswith("bucket= cwd=>")
    ...
    def test_bucket(self, s3_conn: S3, s3_test_setup):
        ...
        try:
            ...
            assert (
                False
            ), f"While setting bucket property to {NEW_BUCKET} the following exception was raised:\n{exc}"
    ...
    def test_compose_s3_url(self):
        url = S3.compose_s3_url(bucket=TEST_BUCKET, key=JSON_FILE)
    ...
    def test_copy_file(self, s3_conn: S3, s3_test_setup):
        ...
        s3_conn.copy_file(copy_from=copy_from, copy_to=copy_to)
        found_objects = [thing for thing in s3_conn.list_objects("", delimiter="")]
    ...
    def test_copy_files(self, s3_conn: S3, s3_test_setup):
        ...
        s3_conn.write_json(target="multi_copy/ex3.json", content={"some": "stuff3"})
        files = [
    ...
    def test_copy_folder(self, s3_conn: S3, s3_test_setup):
        ...
        assert sorted(
            [
                content.split("/")[-1]
                for content in s3_conn.list_folder_contents(
    ...
    def test_create_bucket(self, s3_conn: S3):
        ...
        s3_conn.create_bucket(bucket_name, location="loco", access_control="private")
        assert bucket_name in s3_conn.list_buckets()
    ...
    def test_delete_bucket(self, s3_conn: S3, s3_test_setup):
        ...
        assert response.get("ResponseMetadata", {}).get("HTTPStatusCode") == 204
    ...
    def test_delete_file(self, s3_conn: S3, s3_test_setup):
        ...
        assert (
            response
            and response.get("ResponseMetadata", {}).get("HTTPStatusCode") == 204
        )

    def test_delete_folder(self, s3_conn: S3, s3_test_setup):
        starting_files = [
            file for file in s3_conn.list_folder_contents(path=SUB_FOLDER, delimiter="")
        ]
    ...
    def test_download_folder(self, s3_conn: S3, s3_test_setup):
        ...
        with TemporaryDirectory(dir="./") as temp_dir:
            s3_conn.download_folder(target=SUB_FOLDER, local_path=temp_dir + "/")
            assert os.path.exists(temp_dir + "/" + SUB_FOLDER_FILE)
            s3_conn.download_folder(
                target=SUB_FOLDER, local_path=temp_dir + "/inner-temp/"
            )
            assert os.path.exists(temp_dir + "/inner-temp/" + SUB_FOLDER_FILE)

    def test_download_to_file(self, s3_conn: S3, s3_test_setup):
        with NamedTemporaryFile() as temp_file:
            s3_conn.download_to_file(
                s3_target=SUB_FOLDER_FILE, local_target=temp_file.name
            )
    ...
    def test_download_to_files(self, s3_conn: S3, s3_test_setup):
        ...
        for file_name in file_names:
            s3_conn.write_to_file(filename=file_name, content=b"how you like that")
        with TemporaryDirectory() as temp_dir:
            s3_conn.download_to_files(
                zip(
                    [os.path.join(temp_dir, filename) for filename in file_names],
                    file_names,
                )
            )
            ...
            for _, _, found_files in os.walk(temp_dir):
                assert sorted(found_files) == sorted(
                    file_names
                ), f"actual found files: {found_files}"

    def test_download_to_filelike(self, s3_conn: S3, s3_test_setup):
        with NamedTemporaryFile() as temp_file:
            s3_conn.download_to_filelike(s3_target=SUB_FOLDER_FILE, filelike=temp_file)
            temp_file.seek(0)
    ...
    def test_download_to_filelikes(self, s3_conn: S3, s3_test_setup):
        ...
        for file_name in file_names:
            s3_conn.write_to_file(filename=file_name, content=file_content)
        with TemporaryDirectory() as temp_dir:
            ...
            s3_conn.download_to_filelikes(zip([file_a, file_b, file_c], file_names))
    ...
    def test_list_buckets(self, s3_conn: S3, s3_test_setup):
        ...
        buckets = s3_conn.list_buckets()
        assert sorted(buckets) == sorted([TEST_BUCKET, "another", "and-another"])
    ...
    def test_list_folder_contents(self, s3_conn: S3, s3_test_setup):
    ...
    def test_list_objects(self, s3_conn: S3, s3_test_setup):
        ...
        assert objs
        assert sorted([obj for obj in objs]) == sorted([JSON_FILE, SUB_FOLDER_FILE])
        objs = s3_conn.list_objects(prefix=SUB_FOLDER)
        assert sorted([obj for obj in objs]) == sorted([SUB_FOLDER_FILE])
        s3_conn._bucket = ""
        assert not s3_conn.bucket, s3_conn
        with pytest.raises(Exception):
            # list_objects returns a generator,
            # so we need to peek in to find the error
            s3_conn.list_objects(prefix=SUB_FOLDER).__next__()
    ...
    def test_upload_files(self, s3_conn: S3, s3_test_setup):
        content = b""
        s3_locations = ["file_1.txt", "file_2.txt", "file_3.txt"]
        with TemporaryDirectory() as temp_dir:
            file_names = [
    ...
    def test_upload_folder(self, s3_conn: S3, s3_test_setup):
        with TemporaryDirectory() as temp_dir:
            ...
            s3_conn.upload_folder(target=folder_upload, local_path=temp_dir)
            contents = [
                content
        ...
        with pytest.raises(Exception):
            s3_conn.upload_folder(target="", local_path="./tests/conftest.py")
    ...
    def test_write_json(self, s3_conn: S3, s3_test_setup):
        ...
        response = s3_conn.write_json(target=new_json, content=json_content)
        assert response
    ...
    def test_write_to_file(self, s3_conn: S3, s3_test_setup):
        ...
        response = s3_conn.write_to_file(filename=filename, content=file_content)
        assert response
    ...
    def test_list_json_files(self, s3_conn: S3, s3_test_setup):
        ...
        for new_file in extra_subfolder_json + extra_toplevel_json:
            ...
            s3_conn.write_json(target=new_file, content={"some": "json"})
        # should be all files because no delimiter
        found_json_files = [
            file for file in s3_conn.list_json_files(path="", delimiter="")
        ]
        expected_json_files = [JSON_FILE] + extra_subfolder_json + extra_toplevel_json
        ...
        expected_json_files = [JSON_FILE] + extra_toplevel_json
        for json_file in found_json_files:
            assert json_file in expected_json_files
        assert len(found_json_files) == len(expected_json_files)
        ...
        found_json_files = [
            file for file in s3_conn.list_json_files(path=SUB_FOLDER, delimiter="/")
        ]
        expected_json_files = extra_subfolder_json
        for json_file in found_json_files:
            assert json_file in expected_json_files
        assert len(found_json_files) == len(expected_json_files)
        ...
        found_json_files = [
            ...
            for file in s3_conn.list_json_files(
                path="", delimiter="/", filters={"starts_with": "extra"}
            )
        ]
        expected_json_files = ["extra.json"]
        for json_file in found_json_files:
            assert json_file in expected_json_files
        assert len(found_json_files) == len(expected_json_files)
    ...
    def test_list_csv_files(self, s3_conn: S3, s3_test_setup):
        ...
        for new_file in extra_subfolder_csv + extra_toplevel_csv:
            ...
            s3_conn.write_json(target=new_file, content={"some": "csv"})
        # should be all files because no delimiter
        found_csv_files = [
            file for file in s3_conn.list_csv_files(path="", delimiter="")
        ]
        expected_csv_files = extra_subfolder_csv + extra_toplevel_csv
        ...
        expected_csv_files = extra_toplevel_csv
        for csv_file in found_csv_files:
            assert csv_file in expected_csv_files
        assert len(found_csv_files) == len(expected_csv_files)
        ...
        # should only be subfolder files
        found_csv_files = [
            file for file in s3_conn.list_csv_files(path=SUB_FOLDER, delimiter="/")
        ]
        expected_csv_files = extra_subfolder_csv
        for csv_file in found_csv_files:
            assert csv_file in expected_csv_files
        assert len(found_csv_files) == len(expected_csv_files)
    ...
    def test_get_presigned_url(self, s3_conn: S3, s3_test_setup):
        url = s3_conn.get_presigned_url(
            "put_object",
            params={"Bucket": s3_conn.bucket, "Key": "example_for_url.txt"},
        )
        assert url
        assert type(url) == str
